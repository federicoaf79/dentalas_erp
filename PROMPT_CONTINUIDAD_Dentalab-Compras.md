...
**Actualizado: 21 de agosto de 2026 · Reconciliado contra repo el 5 de agosto de 2026 · Validación en vivo (Chrome) el 7, 10, 11 y 21 de agosto de 2026 · Deploy + validación de limpieza de alertas Etapa 1: 11 de agosto de 2026**
...
>
> **Más tarde el mismo 19/8/2026 — limpieza cerrada, Vercel conectado a Git, y Fase 2 completa de Reposición interna (cálculo + persistencia con estado + pantalla).** Se cerraron los 3 pendientes de limpieza: `yiqi-compras-schema.json` sacado del repo y guardado en el Project; se confirmó que `yiqi_config` **no tiene** fila basura (solo 1 fila real — la nota anterior estaba desactualizada); y se encontró la causa real del webhook de Vercel — **el proyecto nunca había estado conectado a ningún repo de Git** (no un webhook roto). Se conectó `federicoaf79/dentalas_erp` desde Vercel → Settings → Git, y un push de prueba confirmó el auto-deploy funcionando por primera vez en todo el proyecto. Ver **punto 10** para el detalle de la limpieza. Después arrancó el diseño de **Reposición interna** (especificación de Aris): **Fase 2a** — función `reposicion_interna()` con el cálculo Local↔Central, ABC y las 8 prioridades (con un bug real de datos sucios de YiQi encontrado y corregido: `stock_central` negativo generaba cantidades a mover negativas — se agregó un piso en 0); **Fase 2b** — tabla persistente `reposiciones_sugeridas` con estado (pendiente/movido/descartado/vencido), para que la acción de "mover stock" quede auditada y no dependa de un cálculo que cambia todo el tiempo, y pantalla nueva `ReposicionInterna.jsx` en el sidebar con esa acción. Ver **punto 11** para el detalle completo.
>
> **20/8/2026 — Envío de la OC por WhatsApp, semi-automático, construido y validado en vivo.** El botón 💬 en una orden aprobada (Órdenes de compra) descarga un PDF real (sumamos jsPDF — el PDF viejo era un diálogo de impresión, no un archivo) y abre WhatsApp con el texto de la plantilla ya completado con los datos reales de la orden; el único paso manual es arrastrar el PDF a la conversación, porque WhatsApp no permite adjuntar por link. Se encontró y corrigió en el camino un bug real de producción (interop de `jspdf-autotable` con el build de Vite) — validado con un segundo deploy y una prueba en vivo de punta a punta (caso sin WhatsApp cargado y caso con WhatsApp cargado, sin dejar datos de prueba). Ver **punto 14**.
>
> **Todavía el mismo 19/8/2026 — Stock cerrado del todo, sidebar reorganizado en módulos, y accesos reales de Aris/Ivana habilitados.** Se confirmó en vivo (captura) el ciclo completo ✓ Movido de punta a punta: acción sobre el SKU `6245-S` → banner de confirmación → contador bajó de 547 a 545 pendientes, "Quiebre total A" de 8 a 6 — la fila desaparece de la vista y el estado queda escrito en `reposiciones_sugeridas`. **Aclarado con Federico:** esta acción es 100% interna a Dentalab-Compras, **no escribe nada en YiQi** (distinto del flujo de Órdenes de Compra, que sí llama a `enviar-oc-yiqi`) — si conviene que también genere un movimiento de stock en YiQi es una pregunta para Aris, no se decidió unilateralmente (ver ESPERA A OTRA PERSONA). Por pedido de Federico ("el desarrollo puede llegar a crecer con más módulos"), se reorganizó el sidebar de una lista plana a **dos módulos separados** — Stock (Monitor de stock, Reposición interna) y Compras (Alertas al frente, Órdenes de compra, Seguimiento, Historial, Nueva OC) — con badges cableados por `key` en vez de por posición en el array, para evitar el bug de desincronización que ya había pasado una vez esta misma sesión. Ver **punto 12**. Por último, se agregó un botón de **reseteo de contraseña admin-only** ("🔑 Contraseña") en Usuarios y accesos, y Federico lo usó para habilitar el login real de Aris (`aris@dentalab.com.ar`) e Ivana (`comprasdentalab@gmail.com`) con la misma clave que la demo (`Dentalab2026!`) — **confirmado por Federico: "testeado y funcionando"**. Ver **punto 13**.
>
...
>
> **Actualización 21/8/2026 — Remitos de reposición interna (Excel) construidos y validados en vivo; sync de `MOVIMIENTO_STOCK` (último movimiento por SKU) construido, con 3 bugs reales encontrados y corregidos en el camino.** Sobre Reposición interna: nueva vista "Remitos" que agrupa las sugerencias pendientes en tandas de hasta 30 artículos por prioridad, numeradas en secuencia, con exportación a Excel por remito y "todos juntos" (SKU siempre como texto, nunca número) — validado en vivo con Chrome: 21 remitos, 539 artículos, conteos coincidiendo con la vista Lista. Sobre `MOVIMIENTO_STOCK` (Etapa 2 de la limpieza de alertas, ítem 16 de "LO QUE SIGUE"): se construyó el backfill acotado (últimos ~3 años, retomable por cursor) que guarda solo el movimiento más recientes de cada SKU en `ultimo_movimiento_stock_yiqi` — la base de datos para automatizar la regla de "3 años sin movimiento" más adelante, la automatización en sí todavía no está construida. En el camino aparecieron y se corrigieron 3 bugs reales, todos encontrados con evidencia (`net._http_response`, logs de la Edge Function), no adivinados: (1) el guardado de progreso vivía solo al final de las 300 páginas del backfill — un corte a mitad de camino perdía todo el avance, corregido a guardado incremental cada 5.000 filas + al cortarse por cualquier motivo; (2) el timeout de `pg_net` (default 5s) era insuficiente para el volumen real, ajustado a 120s más un timeout de 20s por página individual para no quedar colgado en silencio; (3) el mismo SKU puede repetirse muchas veces dentro de un lote (es un historial, no una tabla de una fila por artículo) — el `DISTINCT ON` agregado en SQL no alcanzó a resolverlo con datos reales (un test manual sí funcionó, la causa exacta no se llegó a reproducir), así que se blindó también con dedupe en JavaScript antes de mandar el lote a Postgres, y ahí sí quedó resuelto — confirmado en vivo (1892 filas guardadas y subiendo, cursor avanzando sin error). De paso se corrigió un build roto (`jspdf`/`jspdf-autotable` nunca habían quedado declarados en `package.json`, un `npm install` limpio los podó de `node_modules`). Ver **SESIÓN 21/8/2026**, al final del documento.
>
> **Más tarde el mismo 21/8/2026 — reconciliación puntual: 2 ítems de "LO QUE SIGUE" estaban DESMENTIDOS.** Al preguntar "¿cómo seguimos?", antes de proponer un menú de pendientes se cruzó la lista contra repo + producción en vivo (protocolo de reconciliación) y aparecieron dos ítems marcados como abiertos que ya estaban 100% construidos y en producción desde hacía días, sin haberse tachado nunca: **ítem 3** (Escritura en YiQi, en realidad cerrada el 18-19/8) e **ítem 5** (Comparación de precios, en realidad cerrada el 20/8). Los dos se verificaron con evidencia de repo Y en vivo con Chrome (no solo código) antes de corregir el documento — ver ítems 3 y 5 de "LO QUE SIGUE" para el detalle. Los ítems 7 (declarar causas) y 13 (editar OC aprobada) sí se confirmaron genuinamente abiertos (sin código relacionado en el repo). Tulkas quedó descartado como fuente para esta corrección puntual: su última auditoría es del 18/8, previa a ambos cierres, así que sus 165 riesgos abiertos incluyen varios ya resueltos — no se tocó Tulkas en esta sesión.
>
...
- `[chrome-live 2026-08-07]` Confirmado en vivo: ambas cuentas reales (Ivana y Aris) ya existen en Supabase Auth con su fila resuelta en `usuarios_config`. `[19/8/2026]` **Login real habilitado y confirmado funcionando** — ver punto 13. Ambos pueden entrar con su mail y `Dentalab2026!`.
...
| 4 | OC generada, aprobada, **y enviada al proveedor** | ✅ **COMPLETO** — `[20/8/2026]` envío semi-automático por WhatsApp (PDF real + texto listo, un paso manual de arrastrar el PDF). Ver punto 14 |
...
1. ~~Envío de la OC al proveedor (Resend / WhatsApp).~~ **✅ WhatsApp resuelto 20/8/2026** — semi-automático (PDF real descargado + WhatsApp con texto listo, un paso manual de arrastrar el PDF), construido y validado en vivo. Ver punto 14. El envío por **email (Resend)** sigue sin construir — no se pidió, no es parte de esta ronda.
...
1. ~~Crear los usuarios reales en Supabase Auth + su fila en `usuarios_config`, y habilitar su login.~~ **✅ HECHO 19/8/2026** — se agregó un reset de contraseña admin-only (botón "🔑 Contraseña" en Usuarios y accesos) y se fijó `Dentalab2026!` en las cuentas reales de Aris (`aris@dentalab.com.ar`) e Ivana (`comprasdentalab@gmail.com`). Confirmado por Federico: "testeado y funcionando". Ver punto 13.
2. ~~Envío de la OC al proveedor.~~ **✅ HECHO 20/8/2026 (WhatsApp)** — ver punto 14. Falta cargar el WhatsApp de pedidos de los proveedores reales (Aris/Ivana, ver ESPERA A OTRA PERSONA) y, si hace falta más adelante, el envío por email (Resend) — no pedido todavía.
3. ~~🔴 Escritura en YiQi (`POST /ORDEN_DE_COMPRA`).~~ **✅ HECHO — este ítem estaba desactualizado, encontrado y corregido en la reconciliación del 21/8/2026.** El mapeo de campos y la integración en el flujo (que este ítem daba como pendiente) ya estaban construidos desde el 18-19/8 (ver SESIÓN 18/8/2026 y 19/8/2026) — nunca se tachó acá. `[repo 2026-08-21]` `components/OrdenesPropias.jsx` muestra "Vinculada a YiQi (OC #...)"/"⚠ Error de vinculación a YiQi" con `yiqi_id_creado`/`yiqi_error`; `supabase/functions/enviar-oc-yiqi/index.ts` existe y fue tocado el 20/8 (fix de duplicación, ver SESIÓN 20/8/2026 sección 3). `[en vivo 2026-08-21, Aris en /]` "Órdenes de compra" muestra 6 órdenes reales vinculadas (#5 a #9, con "Reenviar"/"WhatsApp"), la OC #2572 en YiQi con Asunto "Dentalab-Compras #8" confirmando la escritura real, y la orden #2 con "⚠ Error de vinculación a YiQi" (el caso conocido, pendiente de que Aris confirme duplicados — ver ESPERA A OTRA PERSONA).
...
5. ~~Comparación de precios (`LISTA_DE_PRECIO_COMP` / `PRECIO_ARTICULO_COMP`).~~ **✅ HECHO — este ítem también estaba desactualizado, encontrado y corregido en la reconciliación del 21/8/2026.** Construido el 20/8 (ver SESIÓN 20/8/2026, migraciones `20260820140000`/`20260820150000`/`20260820160000`/`20260820170000`/`20260820180000`) pero nunca se tachó acá. `[repo 2026-08-21]` `ComparacionPrecios.jsx` (545 líneas, algoritmo de similitud por trigramas) y `RevisarEquivalencias.jsx` — ambas en `PAGINAS_CON_DATOS_REALES` de `App.jsx`, no caen en "en construcción". `[en vivo 2026-08-21, Aris en /precios]` búsqueda "guantes" devolvió precios reales y equivalencias con % de coincidencia entre DENTAL MEDRANO y DROVER GUANTES.
...
11. **Módulo de stock por depósito** — entidades confirmadas contra la API real: `STOCK` (no `CONSULTA_DE_STOCK`), `MOVIMIENTO_STOCK` (1.373.423 registros — mucho más que los 25.302 que se había estimado antes), `ACTUALIZACION_DE_STO` (60.616, sin confirmar todavía), `PUNTO_DE_PEDIDO_POR` (punto de pedido por ubicación, solo 5 cargados). `[14/8/2026]` `yiqi-connector` ya tiene el whitelist corregido y las 2 smarties (`STOCK`/2360, `MOVIMIENTO_STOCK`/2359) responden en vivo — falta diseñar las tablas propias y el sync para que esto se automatice, hoy solo se puede consultar a demanda. `[11/8/2026]` Este es el dato que falta para automatizar la regla (a) de la limpieza de alertas (3 años sin compra/ingreso) — ver SESIÓN 11/8/2026, sección 8, Etapa 2. **`[21/8/2026]` Desglose por depósito mostrado en pantalla — ✅ HECHO y validado en vivo.** Hasta ahora `stock_yiqi` se sincronizaba (Fase 1, 19/8) pero no se mostraba en ningún lado — Monitor de Stock y Nueva OC solo mostraban `mate_stock_disponible` (Local+Central combinado, sin discriminar). Se agregó una línea chica "Local: X · Central: Y" debajo del stock combinado en las dos pantallas (Jorge/ML Full en tooltip si tienen algo), vía helper compartido `frontend/src/lib/stockPorDeposito.js`. Ver SESIÓN 21/8/2026.
...
20. `[21/8/2026]` **NUEVO — Conectar "Artículos a pedir" (prioridad 6 de Reposición interna) con el flujo de OC externa.** **✅ HECHO y validado en vivo.** Decisión con Federico (`AskUserQuestion`): solo un puente de navegación, sin unificar los dos cálculos (`reposicion_interna()` y `sugerencias_compra()` siguen siendo independientes). Botón "🛒 Pedir a proveedor" en cada fila de "Artículos a pedir" → salta a Nueva OC con el proveedor y el SKU ya buscados (no agrega nada solo). Pedido explícito de Federico de sumar cartelería/tooltips informativos en toda la acción nueva — se agregaron tooltips en los 4 estados del botón de Acción de Reposición interna y un aviso contextual en Nueva OC explicando de dónde vino la búsqueda. Ver SESIÓN 21/8/2026, sección 5.
21. `[21/8/2026]` **NUEVO — Repaso de UX antes de que Aris e Ivana usaran el sistema un día entero (23/8, sábado).** **✅ HECHO y validado en vivo.** Pedido explícito de Federico ("es primordial que encuentren el sistema bien"), en vez de seguir con los ítems 7/13. Se recorrió el sistema en vivo (no solo el backlog) y salieron 3 problemas activos en producción, los tres cerrados la misma sesión: (a) Monitor de stock calculaba sus propias alertas sin el filtro de exclusiones del ítem 19, mostraba un número más alto todavía que el badge viejo del sidebar; (b) el badge de Alertas del sidebar (`contadores_sidebar()`) tampoco tenía el filtro — 3 lugares de la app mostraban 3 números distintos para la misma cosa; (c) N-10 (input vacío en Reglas guardaba 0 en silencio) seguía abierto desde el 5/8. También se agregó un aviso en "Datos de la empresa" avisando que sigue sin datos reales cargados. Ver SESIÓN 21/8/2026, sección 7.
...
| N-10 | ~~**`ReglasAlertas.jsx:87-89` usa `Number()` sin validar** — un input vacío guarda 0 en `limite_aprobacion` en silencio~~ **✅ Cerrado 21/8/2026** | `[repo 2026-08-05]` `ReglasAlertas.jsx:87-89`. `[repo 2026-08-21]` `guardar()` corta antes de tocar la base si algún campo está vacío/no numérico/negativo, y el botón "Guardar cambios" queda deshabilitado en ese estado. `[en vivo 2026-08-21, Aris en /reglas]` se vació "Límite de aprobación automática" y se clickeó "Guardar cambios": no hubo request, "Última modificación" no cambió. Ver SESIÓN 21/8/2026, sección 7 |
...
5. ~~**Verificar Fase B (RLS) en vivo** → `scripts/verificar-prod.ps1`, **logueado como Ivana en `https://dentalab-compras.vercel.app/login`** (no desde el SQL Editor: ahí `auth.uid()` es NULL). Es lo único que separa "el frontend filtra" de "los datos están protegidas".~~ **✅ HECHO 21/8/2026** — no se usó `verificar-prod.ps1` (sigue sin correrse ese script puntual), pero se validó lo mismo que buscaba de la forma más directa: Federico se logueó como Ivana en `/`, Claude recorrió la app en vivo con Chrome. Confirmado en 7 pantallas: `[en vivo 2026-08-21, Ivana en /]` Monitor de stock (2395/7188 artículos, aviso "19 proveedores asignados"), Alertas (979 vs 2845), Seguimiento de OC (440 vs 932), Órdenes de compra (ve solo su propia OC #2, las 5 de Aris no aparecen, aviso de vista filtrada), Reposición interna (521 vs 1841 — cierra el punto de la sección 10 que decía que la RLS de `reposiciones_sugeridas` nunca se había probado con la cuenta real de Ivana), Usuarios y accesos (bloqueada por completo, "No tenés permiso para ver esta sección"), Reglas y alertas (visible pero de solo lectura, sin botón "Guardar cambios"). Las 7 coinciden con lo esperado — ninguna mostró datos de otro proveedor ni permitió una acción de admin.
...
| `[19/8/2026, NUEVO]` **¿Marcar una Reposición interna como "Movido" debería también generar un registro de movimiento en YiQi** (`MOVIMIENTO_STOCK` o similar), o queda como un registro interno de Dentalab-Compras solamente, como está hoy? | **Aris** | 19/8/2026 | Decisión de diseño explícitamente diferida por Federico ("pedir definiciones a Aris") — no bloquea el uso actual de la pantalla, pero define si hace falta construir una integración de escritura nueva más adelante |
| `[21/8/2026, NUEVO]` **Cargar los datos reales de Dentalab en "Datos de la empresa"** (razón social, CUIT, dirección, teléfono, email, sitio web) | **Aris** | 21/8/2026 | No es código — solo hace falta completar el formulario y guardar. Hasta que se haga, las OC que se generen y se manden por WhatsApp/PDF salen sin membrete completo (sin razón social/CUIT/dirección). Se agregó un aviso ámbar en la pantalla misma que lo recuerda mientras el dato siga vacío |
...
- **Fase 2 (cálculo + pantalla + acción): ver punto 11, más abajo — completa y en producción desde el 19/8/2026.** La RLS nueva (`stock_yiqi`, `reposiciones_sugeridas`) sigue sin probarse en vivo con la cuenta real de Ivana (se armó calcando el patrón ya usado en `material_yiqi`, sin validar con Chrome) — pendiente, no urgente.

### Limpieza pedida 19/8/2026 — ✅ cerrada por completo, más tarde el mismo día

- **`yiqi-compras-schema.json`** (3.97MB, schema OpenAPI de la API Compras de YiQi, sin secretos) — sacado del repo (`git rm --cached` + `.gitignore`, con un detalle de orden importante: el `.gitignore` tiene que ir *antes* del `git rm --cached`, porque un `git add -A` posterior re-agrega el archivo untracked si no está ignorado todavía) y guardado en el Project de Claude (`claude/yiqi-compras-openapi-schema.json`), decisión de Federico vía `AskUserQuestion`.
- **Fila basura en `yiqi_config`** — **descartado, no existía.** Federico corrió el `SELECT` y confirmó **una sola fila real** en la tabla — la nota de la sesión anterior estaba desactualizada (probablemente se refería a una limpieza ya hecha en otra sesión, o a una confusión). No hizo falta tocar nada.
- **Webhook de Vercel desconectado** — la causa real no era un webhook roto: **el proyecto de Vercel nunca había estado conectado a ningún repositorio de Git.** Encontrado en Settings → Git ("This Project is not connected to a Git repository"). Conectado a `federicoaf79/dentalas_erp` desde ahí. Validado con un push real (commit `5f72439`, que incluía esta limpieza + Stock Fase 1): Vercel disparó el deploy solo, sin `vercel --prod` manual, por primera vez en toda la historia del proyecto. **De acá en adelante, `git push` alcanza — ya no hace falta acordarse de deployar a mano.**
- Tabla del MVP: criterio 5 (OC escrita en YiQi) → **completo** desde el cierre de la sesión 18-19/8 (ya actualizado en la tabla de arriba).
...
- ~~Limpiar el archivo suelto `yiqi-compras-schema.json` en la raíz del repo (quedó commiteado sin querer con `git add -A`).~~ **✅ HECHO 19/8/2026** — ver punto 10, nota de limpieza.
- ~~Fila basura en `yiqi_config`~~ **✅ Descartado 19/8/2026** — Federico confirmó una sola fila real, no había nada que limpiar.
- ~~Reconectar el webhook de Vercel~~ **✅ HECHO 19/8/2026** — el problema real era que el proyecto nunca había tenido Git conectado, no un webhook roto. Ver punto 10.

## 11. Módulo de Stock — Fase 2: Reposición interna (cálculo + persistencia con estado + pantalla) — 19/8/2026

**Contexto:** con la Fase 1 (sync de `stock_yiqi`) ya en producción, Federico pidió limpiar/ordenar el módulo Compras para testeo (ver punto 10) y arrancar el módulo de Stock. Confirmó vía `AskUserQuestion` el alcance de esta fase: cálculo Local↔Central + ABC + las 8 prioridades de la especificación de Aris (`ARIS_Especificacion_Reposicion_Interna_y_Produccion.md`), dejando para una fase posterior la sustitución SKU madre/hijo de fraccionados, el export a Excel y las recetas de producción.

### Fase 2a — función `reposicion_interna()`

Reutiliza los mismos building blocks que `sugerencias_compra()`/`es_comprable()` (demanda vía `ventas_mensual_yiqi` + `composicion_articulos` para atribuir venta de combos a sus componentes; exclusión de ML/discontinuados/producción propia). Lógica nueva, específica de Aris:

- Objetivo local = venta mensual promedio (12 meses) = cobertura de 1 mes, fijo — **distinto** de `reglas_compra.meses_cobertura` (esa es la cobertura para comprarle a un proveedor externo, decisión de negocio separada, no se reutiliza a propósito).
- `mover_desde_central = mínimo(déficit local, stock central)` — nunca más de lo que hay en Central.
- Clasificación ABC por participación **acumulada** de venta (A ≤ 80%, B ≤ 95%, C resto), no por venta individual.
- 8 prioridades de la sección 4.4 de Aris, en este orden: Quiebre total A → B → C/presencia mínima → Riesgo alto (cobertura < 7 días) → Completar 1 mes → Artículos a pedir (Central no alcanza) → No enviar al local (vía `es_comprable()`) → No considerados (SKU administrativos `889`/`890`/`99999`, o sin nombre).

**Bug real encontrado y corregido antes de deployar:** `stock_central` puede venir negativo en los datos reales de YiQi (mismo tipo de dato sucio que el `-58` de "En tránsito" ya documentado) — sin un piso, eso generaba `mover_desde_central` **negativo**. Detectado con una consulta de verificación (`sum(mover_desde_central)` por prioridad): la categoría "Sin necesidad" (que por definición nunca debería tener nada que mover) daba `-13` en vez de `0`. Corregido envolviendo `stock_central` con `greatest(stock_central, 0)` en el cálculo. Verificado en vivo tras el fix: "Sin necesidad" pasó a `0/0` exacto — matemáticamente es la única respuesta posible una vez puesto el piso, así que confirma el fix, no solo lo hace plausible.

### Fase 2b — persistencia con estado + acción por fila

Decisión con Federico ("construyamos sólido, para no retocar todo"): `reposicion_interna()` es 100% en vivo, sin memoria — no alcanza para poder "marcar como movido" con auditoría de quién y cuándo, como pide la sección 14 de Aris ("Reposiciones sugeridas":...usuario, estado). Se agregó una capa de persistencia, **solo para las prioridades 1 a 5** (el flujo "mover desde Central al local" — prioridad 6, "Artículos a pedir", es la compra externa, que ya tiene su propio flujo por `sugerencias_compra()`/OC, y mezclarla acá hubiera complicado el campo `estado` sin necesidad todavía).

**Migration `20260819130000_reposicion_sugerida_estado.sql`:**
- `reposicion_interna()` recreada (con `DROP` primero, porque agregar una columna a la tabla de retorno de una función no lo permite `CREATE OR REPLACE`) agregando `proveedor_codigo`, necesario para que la RLS de la tabla nueva pueda usar el mismo patrón dual código/nombre que el resto del sistema.
- **`reposiciones_sugeridas`** — snapshot persistente por SKU (único **pendiente** por SKU a la vez, vía índice único parcial) con `estado` (`pendiente`/`movido`/`descartado`/`vencido`), `cantidad` (sugerida) y `cantidad_movida` (real, si difiere — por ejemplo, si no alcanzaba el stock). RLS calcada de `stock_yiqi`/`material_yiqi` (admin ve todo, el resto solo sus proveedores asignados). Columna `remito` ya presente pero `NULL` — para no tener que tocar el esquema cuando se construya la agrupación en tandas de 30 (fase futura, todavía no arrancada).
- **`generar_reposicion_interna()`** (`SECURITY DEFINER`, corre para todos los proveedores) — idempotente: no duplica sugerencias pendientes por SKU, refresca las que siguen vigentes con el dato más reciente, y cierra como `vencido` las que ya no aplican (se cubrió el déficit, cambió el stock). Cron diario (`generar-reposicion-interna-diario`, jobid 10, 10:00 UTC = 7:00 ART) como red de seguridad, más un botón "↻ Actualizar" en la pantalla para forzarlo al instante.
- **`marcar_reposicion_sugerida(id, estado, observación, cantidad_movida)`** — **no** es `SECURITY DEFINER`: se apoya en la política RLS de `UPDATE` de la tabla para que cada usuario solo pueda marcar sugerencias de sus proveedores asignados.
- Verificado en vivo: `select generar_reposicion_interna()` pobló **547 filas pendientes**, coincidiendo exacto con la suma de las 5 prioridades accionables de la última corrida de `reposicion_interna()` (8+37+222+44+236 tras el resync de stock).

**Frontend — `ReposicionInterna.jsx` (pantalla nueva) + `App.jsx`/`Sidebar.jsx`:** combina dos fuentes — `reposiciones_sugeridas` (prioridades 1-5, con botones **✓ Movido** y **✗ Descartar**, cada uno con su propio modal, no `window.prompt`) y `reposicion_interna()` en vivo (prioridades 6-7, informativas). Prioridades 8/9 ("No considerados"/"Sin necesidad", ~5400 de los ~7150 SKUs) ocultas por defecto — decisión de Federico, con checkbox para mostrarlas. Filtros por prioridad con contador, buscador por SKU/nombre/proveedor, paginado (mismo patrón que `Alertas.jsx`).

**Bug de UI encontrado por Federico y corregido el mismo día:** el input de "cantidad realmente movida" pre-cargaba el float crudo de Postgres (`230.41666666666666`) en vez de un número prolijo. Se agregó un helper `numInput()` (redondeo a 2 decimales, sin separador de miles — un `<input type="number">` no lo acepta) para el input, y se llevó **todos** los números de la pantalla (stock, cantidades, contadores de las pestañas) a formato `es-AR` con separador de miles y 2 decimales vía el helper `num()` ya existente (antes algunos usaban 0, otros 1 decimal, sin separador).

**Deployado y validado en vivo por Federico, con capturas** — commit inicial (Fase 2b + pantalla) y un segundo commit con el fix de formato de números, ambos con auto-deploy de Vercel (funcionando desde la reconexión del punto 10). Capturas confirmaron: 1791 filas totales en la vista por defecto (= suma exacta de prioridades 1-7), filtros funcionando, modal de "Marcar como movido" abriendo con los datos correctos de la fila.

**Pendiente de esta fase:**
- ~~Confirmar en vivo el ciclo completo ✓ Movido / ✗ Descartar (que la fila desaparezca de la lista al confirmar)~~ **✅ HECHO 19/8/2026** — Federico marcó 2 SKUs reales como movidos en producción; captura muestra el banner "6245-S marcado como movido.", el contador de pendientes bajó de 547 a 545 y "Quiebre total A" de 8 a 6. Confirma el ciclo completo (UPDATE con RLS, remoción optimista de la fila en el frontend).
- **Aclaración de arquitectura, no un pendiente técnico:** marcar una sugerencia como "Movido" es una operación 100% interna (solo escribe en `reposiciones_sugeridas`) — **no llama a ninguna API de YiQi**, a diferencia del flujo de Órdenes de Compra (`enviar-oc-yiqi`). Si conviene que también quede un registro de movimiento en YiQi (`MOVIMIENTO_STOCK` o similar) es una decisión de Aris, no se construyó nada especulativo — ver ESPERA A OTRA PERSONA.
- Validar la RLS de `reposiciones_sugeridas`/`stock_yiqi` con la cuenta real de Ivana (mismo pendiente heredado de la Fase 1).
- Fases futuras, explícitamente diferidas: sustitución SKU madre/hijo para fraccionados, agrupación en remitos de hasta 30 artículos + export a Excel, recetas de producción/fraccionamiento, integración de "Artículos a pedir" (prioridad 6) con el flujo de compra externa.

## 12. Reorganización del sidebar en módulos (19/8/2026)

Federico pidió evaluar el menú lateral pensando en que el desarrollo va a seguir creciendo con más módulos ("Igualmente terminemos Stock, y evaluemos si es necesario un ajuste del menú, para tener Separados los módulos de Stock y de compras"). Hasta este punto, "Monitor de stock", "Reposición interna" y "Alertas" vivían sueltos bajo una sección genérica "Principal", sin ninguna agrupación real por módulo de negocio.

**Decisión (`AskUserQuestion`):** Alertas queda dentro de **Compras**, no de Stock — es la pantalla que dispara la necesidad de comprarle a un proveedor externo, mismo criterio de negocio que "Nueva OC"/`sugerencias_compra()`, no un dato de inventario en sí.

**`Sidebar.jsx` reescrito:**
- `NAV_STOCK` = Monitor de stock, Reposición interna.
- `NAV_COMPRAS` = Alertas (ahora primero), Órdenes de compra, Seguimiento de OC, Historial de OC, Nueva OC.
- `NAV_INTELIGENCIA` y `NAV_CONFIG` sin cambios.
- Los badges (contadores del sidebar) se cablearon a un objeto `badgesPorKey = { stock:..., alertas:..., ocs:..., seguimiento:... }` + un `renderGrupo(items)` genérico que busca el badge por `item.key`, en vez del wiring anterior por posición fija en el array (`NAV_PRINCIPAL[1]`, `[2]`...). El cambio es deliberado: en el camino a este rediseño se detectó en la propia sesión un bug real de badge desincronizado al insertar "Reposición interna" en el medio del array viejo — el nuevo patrón hace que agregar/reordenar un ítem de un módulo no pueda romper qué badge le corresponde a cuál.

**Deployado y confirmado en vivo por Federico (captura):** sidebar en producción mostrando las secciones "STOCK" y "COMPRAS" con el agrupamiento nuevo.

## 13. Reset de contraseña admin-only y habilitación de accesos reales (19/8/2026)

**Pedido de Federico:** *"necesito que le habilitemos de nuevo los usuarios y pasword para entrar al sistema con sus mails"* — Aris e Ivana tienen cuenta real en Supabase Auth desde el 7/8 (`aris@dentalab.com.ar`, `comprasdentalab@gmail.com`) pero nadie conoce su contraseña original, así que en la práctica seguían usando las cuentas demo.

**Decisión (`AskUserQuestion`):** ponerles la misma clave que la demo (`Dentalab2026!`) en vez de armar un flujo de reset por mail — apropiado para un sistema interno de pocos usuarios, sin la complejidad de configurar envío de mails en producción.

**Backend — `admin-usuarios` Edge Function, acción nueva `resetPassword`:** `POST ?accion=resetPassword` con `{ userId, password }`, usa `supabaseAdmin.auth.admin.updateUserById(userId, { password })` (API de administración de Supabase, service_role) para fijar la contraseña directo, sin mandar mail ni pedir la anterior. Protegida por el mismo gate `verificarLlamador({ soloAdmin: true })` que ya cubre el resto de la función — sin código de autorización nuevo.

**Frontend — `UsuariosAccesos.jsx`:** botón nuevo "🔑 Contraseña" junto a "Gestionar accesos" en cada fila de usuario, abre un modal propio (mismo patrón visual que el resto de la app, no `window.prompt`) pre-cargado con `Dentalab2026!` (editable), con aviso de éxito transitorio.

**Contratiempo de deploy, encontrado y corregido en la misma sesión:** el primer intento de deploy/commit salió con el código viejo — el puente con la compu de Federico (`device_commit_files`) había estado caído justo cuando se mandaron los archivos por chat, y sin el paso manual de guardarlos, `git commit` reportó "no changes to commit" para los dos archivos editados, y el deploy de la función mandó el código anterior. Se detectó releyendo literalmente la salida pegada por Federico (no se asumió éxito), se reescribieron los archivos directo por el puente ya reconectado, y se repitió el ciclo completo (`supabase functions deploy admin-usuarios` + `git add`/commit/push) — esta vez limpio, commit `31730f0`.

**✅ Confirmado por Federico, en producción: "listo, testeado y funcionando."** Aris y Ivana ya pueden entrar a `dentalab-compras.vercel.app` con su mail real y `Dentalab2026!`.

**Alcance actual, explícito (sin sobre-construir):** no existe todavía autoservicio de cambio de contraseña para un usuario ya logueado, ni recuperación por "olvidé mi contraseña" (requeriría configurar SMTP/`resetPasswordForEmail` en Supabase Auth, no confirmado como armado en producción). Hoy, tanto para cambiar como para un olvido, el único camino es que un admin repita el reset con el botón 🔑. Evaluado con Federico y dejado así a propósito por ahora — suficiente para un equipo interno de 2-3 usuarios; queda anotado como mejora futura, no bloqueante.

## 14. Envío semi-automático de la OC por WhatsApp (19-20/8/2026)

**Contexto:** retomando el pendiente de "Envío de la OC al proveedor" (criterio 4 del MVP, parcial desde el inicio del proyecto), Federico confirmó por `AskUserQuestion` el diseño ya acordado antes de la reorganización de Stock: **semi-automático** — el sistema descarga el PDF y abre WhatsApp con el texto ya armado; la persona arrastra el PDF una vez a la conversación. No Meta Business API (eso queda aparte, requiere cuenta verificada, plantillas aprobadas, costo por conversación — no es "ya mismo").

**Problema técnico encontrado antes de construir:** el PDF de la orden (`pdfOrden.js`) no era un archivo real — abría una ventana con "Imprimir/Guardar PDF" que requería un paso manual del navegador. Para que el botón descargue solo hacía falta un PDF real. Confirmado con Federico (`AskUserQuestion`): se suma **jsPDF** (+ `jspdf-autotable` para la tabla de ítems) en vez de mantener el enfoque de impresión.

**Construido:**
- `pdfOrden.js` — nueva función `generarPdfOrdenDescargable()` (jsPDF), en paralelo a la vieja `generarPdfOrden()` (impresión, sin tocar, sigue usándose en el botón "PDF"/"Descargar PDF" de siempre). Arma el mismo contenido (empresa, proveedor, tabla de ítems, total, aviso de "sin costo", notas) pero como archivo `.pdf` real, con `doc.save()` — descarga sola, sin diálogo.
- `OrdenesPropias.jsx` — botón nuevo **💬 WhatsApp** (fila y panel de detalle) para órdenes en estado `aprobada`. Al click: trae los ítems reales, resuelve el WhatsApp del proveedor (`proveedores.whatsapp_pedidos`, cargado en Condiciones comerciales) y si no está cargado corta con un aviso claro en vez de fallar en silencio; descarga el PDF real; arma el texto con la plantilla `tpl_wa_oc` (la misma que ya usaba Nueva OC) con los datos reales de la orden ya guardada (no el placeholder "se asigna al guardar" de Nueva OC); abre `wa.me`; guarda auditoría (`whatsapp_enviada_en`/`whatsapp_enviada_por`) — mismo patrón que `archivada_en`/`archivada_por`. El botón pasa a "💬 Reenviar" una vez enviada, y el panel de detalle muestra un aviso con fecha.
- Migration `20260819140000_whatsapp_envio_oc.sql` — agrega esas dos columnas a `ordenes_propias`. Sin cambios de RLS (la política de UPDATE ya existente cubre las columnas nuevas).
- Textos actualizados para que dejen de mentir: el banner de "Órdenes generadas desde el sistema" (decía "todavía no se envían... al proveedor") y el aviso de `TemplatesMensajes.jsx` (decía "el envío automático... todavía no está implementado").

**🔴 Bug real encontrado en producción, ya corregido:** el primer deploy (commit `12ea93f`) tiraba `(0 , UI.default) is not a function` al clickear 💬 — problema de interoperabilidad ESM/CJS de `jspdf-autotable` específico del build de producción de Vite/Rollup (el import `import autoTable from 'jspdf-autotable'` + llamarlo como función funcionaba en dev, no en prod). **No se detectó con la validación local previa al primer deploy** porque esa validación (esbuild) externalizaba `jspdf-autotable` en vez de empaquetarlo de verdad — lección para la próxima vez que se agregue una librería con esta clase de dependencia. Corregido (commit `cc45a0d`) cambiando al patrón de import de efecto (`import 'jspdf-autotable'` + `doc.autoTable(...)`, no `autoTable(doc, ...)`) — el patrón que la propia librería documenta como más compatible entre bundlers. Verificado antes de redeployar generando un PDF real de punta a punta en Node con los paquetes reales instalados (10.5KB, 1 página, contenido correcto) para no repetir un segundo redeploy a ciegas.

**✅ Validado en vivo, de punta a punta, con Chrome (logueado como Aris), después del segundo deploy:**
- Caso sin WhatsApp cargado (DENTAL MEDRANO, sin dato): click en 💬 → aviso claro "DENTAL MEDRANO no tiene WhatsApp de pedidos cargado. Cargalo en 'Condiciones comerciales'..." — no descarga nada, no abre nada. Camino de error correcto.
- Caso con WhatsApp cargado (número de prueba temporal `+54 9 11 0000-0000` en DENTAL MEDRANO, cargado y **borrado de nuevo al terminar la prueba** — no quedó dato de prueba en `proveedores`, a diferencia de una orden de prueba descartable, ese campo es configuración real que Aris/Ivana pueden usar): click en 💬 sobre la orden real #9 → se abrió `api.whatsapp.com/send` con el número correcto y el texto completado con los datos reales de la orden (proveedor, ítem, total $860) → aviso verde de confirmación → botón pasó a "💬 Reenviar" → panel de detalle mostró "💬 Enviada por WhatsApp el 19/08/2026, 09:50 p. m.".
- La descarga del PDF ocurre en el mismo click (confirmado por el flujo — el navegador la dispara sin diálogo).
...
# SESIÓN 21/8/2026 — Remitos de reposición interna (Excel), y sync de `MOVIMIENTO_STOCK` con 3 bugs reales encontrados y corregidos

## 0. Contexto

Se retomó el análisis de UX del 20/8 (contestado: nada de la auditoría se tocó todavía, a propósito — es la lista de pendientes, no urgencias; los 3 ítems marcados "urgente" ahí son el token de YiQi vencido —ya resuelto en sesiones previas—, "Datos de la empresa" vacío, y la posible duplicación de OC —ya resuelta el 20/8, ver sección anterior—). Con eso aclarado, se arrancó con el orden ya acordado con Federico: primero Stock ítem 1 (remitos), después Stock ítem 2 (sync de movimientos).

## 1. Remitos de reposición interna — tandas de hasta 30 artículos + Excel

**Regla de negocio (especificación de Aris):** agrupar las sugerencias pendientes de Reposición interna en remitos de hasta 30 artículos, por prioridad, numerados en secuencia a través de todas las prioridades ("Remito 01 - Quiebre A", "Remito 02 - Quiebre A", "Remito 03 - Quiebre B"...). Columnas del Excel: SKU / Descripción / Cantidad — **SKU siempre como texto**, nunca número (mismo criterio de siempre: hay SKU tipo "31110 T").

**Construido:**
- **Migration `20260821090000_remitos_reposicion.sql`** — `generar_remitos_reposicion()` (`SECURITY DEFINER`): asigna el `remito` a cada fila pendiente de `reposiciones_sugeridas`, agrupando de a 30 dentro de cada prioridad (`row_number()` particionado por prioridad, ordenado por venta de 12 meses descendente) y numerando en secuencia con `dense_rank()`. Recalcula todo en cada corrida (no es incremental).
- **`frontend/src/lib/exportarExcel.js`** (nuevo) — `exportarRemitoExcel()` / `exportarTodosLosRemitosExcel()` con SheetJS (`xlsx`), forzando el tipo de celda del SKU a texto (`t:'s'`) después de armar la hoja, para que Excel no le adivine formato de número a SKU como "889". Sanitiza nombres de hoja (sin `:\/?*[]`, máx 31 caracteres, dedupe con sufijo si hace falta).
- **`ReposicionInterna.jsx`** — toggle Lista/Remitos, botón "↻ Generar remitos" (llama al RPC), botón "⬇ Descargar todos (Excel)", y una tarjeta por remito con su tabla y botón de descarga individual.
- **`package.json`** — se sumó `xlsx` como dependencia nueva.

**Validado en vivo con Chrome:** 539 filas pendientes → 21 remitos generados; "Remito 01 - Quiebre total A" con 9 artículos / 137,25 unidades, coincidiendo con el conteo de la vista Lista; Excel descargado sin errores de consola.

## 2. Sync de `MOVIMIENTO_STOCK` (último movimiento por SKU) — Etapa 2 de la limpieza de alertas (ítem 16)

**Alcance, decisión con Federico (`AskUserQuestion`):** NO se sincroniza el historial completo (1,38M+ filas y creciendo) — solo se guarda el movimiento MÁS RECIENTE de cada SKU, que es lo único que hace falta para automatizar más adelante la regla de "3 años sin movimiento". La automatización en sí queda para después; esta sesión fue solo la base de datos.

**Investigación en vivo antes de construir** (misma técnica de siempre — token del navegador vía consola): la smartie `MOVIMIENTO_STOCK` (id 2359) viene ordenada por fecha de alta DESCENDENTE, 50 filas por página (no 100 como las smarties core), y el `id` de cada fila es correlativo en el mismo sentido que la fecha — permite un backfill ACOTADO escaneando de la más nueva hacia atrás, cortando apenas se cruzan ~3 años de antigüedad, en vez de traer el millón y medio de filas completo.

**Construido (primera versión):**
- **Migration `20260821110000_movimientos_stock.sql`** — cursor de página + flag de "backfill completo" en `yiqi_config`; tabla `ultimo_movimiento_stock_yiqi` (1 fila por SKU, RLS igual que `stock_yiqi`); `upsert_ultimo_movimiento_stock_yiqi()` (solo pisa si el movimiento entrante es más nuevo que el guardado); cron cada 15 min.
- **`sync-yiqi/index.ts`** — `sincronizarUltimoMovimientoStock()`: en modo backfill pagina desde el cursor guardado (300 páginas por corrida al principio); en modo incremental siempre relee las páginas más recientes (40 por corrida).

**🐛 Bug 1 — se perdía todo el progreso si la corrida se cortaba a mitad de camino.** El guardado (filas + cursor) vivía solo al final del loop completo de páginas. Las 2 primeras corridas reales (300 páginas cada una) nunca llegaron a ese final — confirmado con `net._http_response`: ambas cortaron justo en el timeout configurado, sin dejar ningún rastro (`count(*) = 0`, cursor en 0). **Fix:** guardado incremental cada 5.000 filas acumuladas + un `finally` que guarda lo que haya pase lo que pase (corte normal, corte de antigüedad, cualquier error) — así una corrida cortada a la mitad deja progreso real en vez de perderlo todo. De paso se bajó de 300 a 150 páginas por corrida (más conservador) y se agregó un timeout de 20s a cada página individual, para que un colgado de YiQi en una sola página no se coma toda la invocación en silencio.

**🐛 Bug 2 — timeout de `pg_net` insuficiente.** El default de `pg_net` (5000ms, usado por TODOS los crons del proyecto, no solo este) alcanza para los syncs chicos pero no para hasta 150-300 páginas seguidas contra YiQi. **Fix (migration `20260821120000_movimientos_stock_pgnet_timeout.sql`):** `timeout_milliseconds := 120000` en el `net.http_post` de este cron puntual. **Nota dejada sin resolver a propósito:** el mismo default de 5s probablemente afecta a `reintentar-oc-pendientes-yiqi` y otros crons del proyecto (se vieron timeouts de 5000ms en `net._http_response` en esos horarios también) — no se tocó porque hoy no causa ningún síntoma real, queda para revisar si alguna vez aparece uno.

**🐛 Bug 3 — el mismo SKU puede repetirse muchas veces dentro de un lote.** A diferencia del resto de las tablas espejo (1 fila por artículo), `MOVIMIENTO_STOCK` es un historial — el mismo SKU aparece una vez por cada movimiento, así que un lote de 5.000 filas consecutivas casi siempre trae el mismo SKU repetido. Postgres no permite que un `INSERT ... ON CONFLICT DO UPDATE` toque la misma fila dos veces dentro de una sola sentencia (error real: `"ON CONFLICT DO UPDATE command cannot affect row a second time"`) — corta la corrida entera. **Primer intento (migration `20260821130000_movimientos_stock_dedup_fix.sql`):** `DISTINCT ON (sku)` dentro de la función SQL de upsert, quedándose con el movimiento más reciente de cada SKU antes de insertar. Un test manual con 2 filas duplicadas confirmó que el `DISTINCT ON` en sí funciona bien — pero con el lote real (5.000 filas de verdad) el mismo error volvió a aparecer, sin poder reproducir la causa exacta a mano (sospecha no confirmada: algo de comparación/colación de texto puntual de esos datos). **Fix definitivo:** dedupe también del lado de JavaScript (`dedupePorUltimoPorSku()` en `sync-yiqi/index.ts`, antes de mandar el lote a la RPC) — comparación de string exacta, sin depender de ningún detalle de Postgres. Con este segundo blindaje sí quedó resuelto.

**De paso, build roto encontrado y corregido:** `pdfOrden.js` (código preexistente, no de esta sesión) importa `jspdf`/`jspdf-autotable`, pero ninguna de las dos había quedado declarada en `package.json` — un `npm install` limpio (necesario para sumar `xlsx`, ver punto 1) las podó de `node_modules` y rompió el build. Se agregaron `jspdf@^2.5.1` y `jspdf-autotable@^3.8.4` a `package.json` (versiones probadas en sandbox contra el uso real del código, `doc.autoTable({...})`, API vieja tipo v3).

**✅ Confirmado en vivo, de punta a punta:** después de los 3 fixes, `ultimo_movimiento_stock_yiqi` con 1.892 filas y subiendo, cursor de backfill avanzando sin error (617 al momento de cerrar esta sesión), `backfill_completo` todavía en `false` (es esperado — recién arrancando, hay mucho volumen). El cron de cada 15 min sigue el backfill solo desde acá, sin necesidad de intervención manual.

**Pendiente, no urgente:** construir la automatización en sí de la regla "3 años sin movimiento" en Alertas usando este dato (hoy sigue siendo exclusión/pausa 100% manual — ver decisión de negocio #14 y SESIÓN 11/8/2026, sección 8, Etapa 2).

## 3. Desglose de stock por depósito en Monitor de Stock y Nueva OC (ítem 11)

**Gap real, confirmado leyendo el código:** `stock_yiqi` se sincroniza automáticamente cada 15 min desde el 19/8/2026 (Fase 1 del módulo de Stock), pero hasta hoy **ninguna pantalla del frontend la consultaba** — un `grep` de `stock_yiqi` en `frontend/src/` no devolvía nada. Monitor de Stock y Nueva OC seguían mostrando solo `mate_stock_disponible` (Local+Central combinado, sin discriminar), que es justo lo que el documento de Aris marca como scope nuevo: *"desglose de stock por depósito en cualquier pantalla (hoy MATE_STOCK_DISPONIBLE es Local+Central combinado, sin discriminar)"*.

**Construido:**
- **`frontend/src/lib/stockPorDeposito.js`** (nuevo) — `traerStockPorDeposito()`: trae `stock_yiqi` completo, paginado igual que `traerMaterialLocal()` (PostgREST topea 1.000 filas por request), **sin filtro de permisos propio** porque la RLS de `stock_yiqi` ya resuelve la visibilidad por usuario (mismo join que `material_yiqi`). `textoDesgloseStock()`: arma el texto "Local: X · Central: Y" + un tooltip con Jorge/ML Full si tienen algo.
- **`MonitorStock.jsx`** — debajo del stock combinado (columna "Stock"), una línea chica gris con el desglose. Se carga en paralelo con `material_yiqi` (`Promise.all`); si falla, no rompe el resto de la pantalla (solo un `console.warn`).
...
## 5. Puente Reposición interna → Nueva OC, con cartelería y tooltips (ítem 20)

**Contexto:** "Artículos a pedir" (prioridad 6 de `reposicion_interna()`, Central tampoco alcanza) era puramente informativo — Aris/Ivana tenían que acordarse del SKU y proveedor e ir a buscarlo a mano en Nueva OC. Consultado con Federico (`AskUserQuestion`, 3 opciones: solo un enlace / unificar el cálculo del faltante / no tocar todavía) — eligió la opción de menor riesgo: un puente de navegación, **sin** tocar ningún cálculo que ya está en producción. Pidió además que toda la acción tenga cartelería, mensajes y tooltips informativos.

**Por qué no es trivial:** la app no usa router — la navegación entre pantallas es un estado `currentPage` en `App.jsx`, sin query params ni historial. Pasar "andá a Nueva OC con este proveedor y este SKU" de una pantalla a otra necesitó una preselección levantada al componente padre.

**Construido:**
- **`App.jsx`** — estado `preseleccionOC` (`{proveedor, sku}` o `null`). `ReposicionInterna` recibe `onPedirAProveedor(proveedor, sku)` que lo setea y cambia `currentPage` a `'nueva-oc'`. `NuevaOC` recibe `preseleccion` + `onConsumirPreseleccion` para limpiarlo después de usarlo (mismo patrón ya usado para Comparación de precios → Revisar equivalencias).
- **`ReposicionInterna.jsx`** — en la columna Acción, las filas de "Artículos a pedir" (6) muestran **"🛒 Pedir a proveedor"** si el artículo tiene proveedor cargado, o "Sin proveedor" (con tooltip explicando que hay que cargarlo en YiQi) si no. Tooltips agregados también en ✓ Movido, ✗ Descartar e "Informativo" (7). Banner de criterio de la pantalla actualizado para explicar el botón nuevo (id renovado a `reposicion-criterio-v2` para que quien ya había cerrado el aviso viejo vea el texto nuevo).
- **`NuevaOC.jsx`** — `elegirProveedor()` ahora puede llamarse con cualquier nombre de proveedor, sin depender de que aparezca en `proveedores_con_alertas()` (lista calculada con otro criterio — `sugerencias_compra()` vs `reposicion_interna()` — podían no coincidir). Al llegar por preselección, salta directo a "armar orden" y precarga el buscador con el SKU (`buscarTexto` + `buscarAbierto`), **sin agregarlo solo** — mismo criterio ya documentado en el código ("el sistema sugiere, la persona decide"): la persona ve el resultado y hace click en "+ Agregar" si quiere. Aviso contextual (ámbar, autocierra a los 20s) explica de dónde vino la búsqueda y por qué. La preselección se limpia sola al elegir otro proveedor a mano después, para no dejar un SKU viejo precargado por error.

**✅ Validado en vivo con Chrome, logueado como Aris:** filtro "Artículos a pedir (820)" en Reposición interna → click en "🛒 Pedir a proveedor" del SKU `976` (MASOTTO DANIEL (VASOS)) → saltó directo a Nueva OC, armando la orden para ese proveedor (bypaseando el selector), con el aviso ámbar mostrando el mensaje esperado y el buscador ya mostrando el resultado "976 — Vasos Descartables Blancos x 100 und." con el botón "+ Agregar" listo. No se guardó ningún borrador ni se dejó ninguna orden de prueba.

## 6. Reconciliación puntual — ítems 3 y 5 de "LO QUE SIGUE" estaban desmentidos

**Disparador:** Federico preguntó "¿cómo seguimos?" tras cerrar los ítems 18-20. Antes de armar un menú de pendientes, se cruzó la lista contra el repo y producción (protocolo de reconciliación de Federico) en vez de asumir que la lista estaba al día.

**Encontrado:**
- **Ítem 3 (Escritura en YiQi)** decía "falta el mapeo de campos reales y la integración en el flujo" — pero eso se había cerrado el 18-19/8 (ver esas sesiones) y nunca se tachó acá. Verificado con evidencia doble: `[repo 2026-08-21]` `grep` en `components/OrdenesPropias.jsx` confirmó `yiqi_id_creado`/`yiqi_error`/"Vinculada a YiQi" ya construidos; `[en vivo 2026-08-21, Aris en /]` "Órdenes de compra" mostró 6 órdenes reales vinculadas y la orden espejo #2572 en YiQi.
- **Ítem 5 (Comparación de precios)** decía solo el nombre, sin marca de hecho — pero se había construido el 20/8 (ver esa sesión, 5 migraciones) y tampoco se tachó. Verificado con `[repo 2026-08-21]` (`ComparacionPrecios.jsx`, 545 líneas, algoritmo de trigramas) y `[en vivo 2026-08-21, Aris en /precios]` (búsqueda real con resultados).

**Por qué pasó (hipótesis, no confirmada):** ambos ítems se construyeron en sesiones enfocadas en otra cosa (18-20/8 tuvieron su propio foco temático) y quedaron documentados en el cuerpo de sus respectivas secciones de sesión, pero el paso de tachar la línea correspondiente en "LO QUE SIGUE, EN ORDEN" se salteó. Mismo patrón de falla que ya había pasado antes con el ítem 12 (ver SESIÓN 11/8/2026) — sugiere que "LO QUE SIGUE" necesita revisarse explícitamente al cerrar cada sesión, no asumir que actualizar la sección de la sesión alcanza.

**Genuinamente verificados como abiertos** (no se tocaron, siguen así): ítem 7 (declarar causas — sin ninguna referencia a `declaraciones_causa` fuera de `CatalogoCausas.jsx`) e ítem 13 (editar OC aprobada — sin ningún código de edición en `OrdenesPropias.jsx`/`NuevaOC.jsx`). Confirmado por `grep` en `frontend/src/pages/` y `frontend/src/components/`.

**Tulkas, descartado como fuente para esta corrección:** se consultó `get_project('dentalab')` como cuarta fuente del protocolo — su última auditoría es del 18/8/2026, anterior a los dos cierres reales, y sus 165 riesgos abiertos incluyen varios ya resueltos (comparación de precios, escritura a YiQi, badge del sidebar, entre otros). Se usó solo como corroboración de que Tulkas también está desactualizado en estos puntos, no como fuente de verdad — no se modificó nada en Tulkas esta sesión.

## 7. Repaso de UX antes del sábado 23/8 (ítem 21)

**Disparador:** Federico, tras cerrar los ítems 18-20: *"Antes de esos 2. Por que no refaccionamos la UX, con todo lo encontrado? Y buscamos mejorarla. mañana Sabado, Aris e Ivana van a trabajar todo el día. y es primordial que encuentren el sistema bien."*

**Encontrado buscando el "análisis de UX del 20/8" mencionado antes en este documento:** ese análisis no quedó nunca como una lista escrita en ningún lado — solo hay referencias sueltas a sus hallazgos (líneas ~1109 y ~1127 de este documento). En vez de reconstruirlo de memoria, se hizo un recorrido en vivo del sistema completo (logueado como Aris) cruzado con el backlog ya documentado (N-10, ítems 7/13). Salieron 2 problemas activos en producción que no estaban en ningún lado todavía:

1. **Monitor de stock calculaba sus propias alertas sin el filtro de exclusiones del ítem 19.** `calcularAlerta()`/`conAlerta` en `MonitorStock.jsx` no excluía SKU administrativos, ML, discontinuados ni producción propia — mismo bug que tenía `Alertas.jsx` antes de hoy, pero en la primera pantalla que ve cualquiera al entrar. Mostraba 2595 crít./443 prev. cuando el número real era 2401/440 (mostrado en la pantalla de Alertas, ya corregida esta tarde).
2. **El badge de "Alertas" del sidebar (`contadores_sidebar()`) tampoco tenía el filtro.** Entre las tres pantallas —sidebar, Monitor de stock, Alertas— había 3 números distintos y ninguno coincidía, para el mismo dato.

Sumado a esto, N-10 (ya documentado el 5/8, seguía abierto): un input vacío en "Reglas y alertas" guardaba `0` en silencio en `limite_aprobacion`/`max_cajas_por_producto`/`meses_cobertura`.

Se presentó el hallazgo a Federico con `AskUserQuestion` antes de tocar nada — eligió las 4 opciones ofrecidas (badge de Alertas, validación en Reglas, seguir recorriendo, y avisar sobre "Datos de la empresa").

**Construido, deployado y validado en vivo, los 4:**

- **`MonitorStock.jsx`** — se agregó `esExcluidoDeAlertas()` (copiado literal de `Alertas.jsx`, con nota de que si se vuelve a tocar el criterio hay que actualizar los dos lugares) y se aplicó en el `useMemo` de `conAlerta`. `[en vivo 2026-08-21, Aris en /]` pasó de 2595/443 a 2407/438, coincidiendo con Alertas.jsx.
- **`contadores_sidebar()`** — función no versionada, aplicada directo en producción hace tiempo. Se pidió su definición en vivo con `pg_get_functiondef` (no se asumió desde el repo) y se agregaron las mismas 4 condiciones de exclusión a la CTE `mat`, migration `20260821150000_fix_contadores_sidebar_exclusiones.sql`, aplicada por Federico vía SQL Editor. `[en vivo 2026-08-21, Aris en /]` el sidebar pasó de 2591/443 a 2403/438 (vs. 2407/438 de la pantalla de Alertas — la diferencia de 4 es drift normal del sync de 15 min entre una carga y otra, no un desacuerdo de criterio).
- **`ReglasAlertas.jsx`** (N-10) — `guardar()` corta antes de tocar la base si algún campo está vacío, no numérico o negativo; el botón "Guardar cambios" queda deshabilitado en ese estado, con `title` explicando por qué. `[en vivo 2026-08-21, Aris en /reglas]` se vació el campo de límite y se clickeó "Guardar cambios": no hubo request a Supabase, "Última modificación" no cambió.
- **`Empresa.jsx`** — se investigó primero si el riesgo era que una OC real saliera con datos falsos ("Av. Siempreviva 742", CUIT "30-12345678-9"): **no es así** — son placeholders HTML de inputs vacíos, no datos guardados, y `pdfOrden.js` omite cada línea si el campo está vacío (no imprime el ejemplo). El riesgo real es que la OC salga sin membrete. Se agregó un aviso ámbar (visible solo si `razon_social`/`cuit`/`direccion` siguen vacíos) explicando esto y pidiendo que se carguen los datos reales antes del sábado. **Sigue pendiente que Aris cargue los datos** — no es una tarea de código, ver ESPERA A OTRA PERSONA.

**No se tocaron** los ítems 7 y 13 (confirmados como genuinamente pendientes en la sección 6) ni la pequeña discrepancia de 925 vs. 926 en "Órdenes de compra" (se verificó que es un dato que cambia en vivo entre una carga de pantalla y otra, no un bug — "Seguimiento de OC" mostró 925 "Enviadas" de forma consistente con el sidebar en la misma carga).

**Verificación de que las 24 migraciones del repo (10/8 al 21/8/2026) están realmente aplicadas en producción.** Pedido de Federico, al cerrar esta sección: no alcanza con confiar en que cada sesión "aplicó lo que dijo aplicar" — hay que confirmarlo contra la base. Como casi todas se aplicaron pegando el SQL a mano en el SQL Editor (no con `supabase db push`), `supabase migration list` no sirve — la tabla que usa el CLI para llevar la cuenta nunca se llenó. Se armó en cambio `scripts/verificar_migraciones_21_8.sql`, que pregunta directo al catálogo de Postgres: existencia de las 8 tablas y 20 funciones que las migraciones crean, más 6 chequeos de que la versión VIVA de las funciones que se reemplazaron varias veces (`sugerencias_compra`, `reposicion_interna`, `contadores_sidebar`, `candidatos_equivalencia_precios`, las 2 de movimientos de stock) tiene el fix de la migración más reciente y no una versión vieja. `[prod 2026-08-21]` Federico lo corrió en el SQL Editor: **28/28 objetos existen, 6/6 chequeos de versión dieron `true`.** Las 24 migraciones están aplicadas y son las vigentes — no hay ninguna que haya quedado pendiente o pisada por una versión anterior.
